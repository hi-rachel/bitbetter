...
- App Router에서는 `fetch`에 `{ cache: 'no-store' }` 옵션시 SSR로 동작합니다. (ver.15 기본값)

### SSG (Static Site Generation)

- 빌드 타임에 미리 HTML을 생성해 정적 파일로 서빙합니다.
- 빠른 응답 속도와 CDN 캐싱에 유리합니다.
- App Router에서는 `fetch`에 `{ cache: 'force-cache' }`를 사용하면 SSG로 동작합니다. (ver.14 기본값)
...
## 2. Next.js 15 이상에서 fetch 기본 동작 변화

- **Next.js 14 이하**: fetch의 기본 옵션은 `{ cache: 'force-cache' }` (SSG, 정적 캐싱)
- **Next.js 15 이상**: fetch의 기본 옵션이 `{ cache: 'no-store' }` (SSR, 매 요청마다 서버 패칭)로 변경됨
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>`fetch`에 `{ cache: 'no-store' }` (ver.15 기본값)</td>
    </tr>
    <tr>
      <td>SSG</td>
      <td>빌드 타임에 정적 HTML 생성, 빠른 응답</td>
      <td>`fetch`에 `{ cache: 'force-cache' }` (ver.14 기본값)</td>
    </tr>
    <tr>
      <td>ISR</td>
